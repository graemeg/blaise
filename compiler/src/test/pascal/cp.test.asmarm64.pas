...
type
  ...
  published
    ...
    { BUG-20260726-arm64-asm-hash-comment — a '#' opening a line is a comment;
      '#' elsewhere is an immediate prefix and must stay one. }
...
procedure TArm64AsmTests.TestHashLineComment_IgnoredButImmediatesKept;
var
  F: TMachOFile;
  T: TMoSection;
begin
  { GNU as treats a '#' that OPENS a line as a comment, and cpp line markers
    ('# 12 "file.pas"') look the same.  The arm64 assembler had no such rule, so
    any '#' line came back as 'unknown instruction: #'.
    The rule can only apply at the START of a line: on AArch64 '#' introduces an
    immediate, so a blanket comment character would eat every immediate operand.
    Both halves are asserted here — the comment lines vanish, and the two
    instructions around them still encode with their immediates intact. }
  T := TextWords(
    '# OPDF debug companion file — generated by Blaise compiler' + LineEnding +
    '# 12 "prog.pas"' + LineEnding +
    'mov x0, #5' + LineEnding +
    '   # an indented comment is still a comment' + LineEnding +
    'add x1, x0, #7' + LineEnding, F);
  try
    { two instructions, four words would mean a comment leaked through }
    AssertEquals('only the two instructions were encoded', 8, Integer(T.Size));
    { mov x0, #5  -> MOVZ x0, #5   = 0xD2800000 or (5 shl 5) }
    AssertEquals('mov immediate survived',
      Integer($D2800000) or (5 shl 5), WordAt(T, 0));
    { add x1, x0, #7 -> 0x91000000 or (7 shl 10) or (0 shl 5) or 1 }
    AssertEquals('add immediate survived',
      Integer($91000000) or (7 shl 10) or 1, WordAt(T, 1));
  finally
    F.Free();
  end;
end;
