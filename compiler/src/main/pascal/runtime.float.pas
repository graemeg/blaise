...
{ ------------------------------------------------------------------ }
{ _StrToDouble -- CORRECTLY ROUNDED decimal -> binary64 conversion     }
{                                                                      }
{ The result is the nearest double to the exact decimal value, ties    }
{ to even -- bit-identical to C strtod / CPython.  This is load-       }
{ bearing well beyond user code: the INTERNAL ASSEMBLER converts every }
{ float literal of every Blaise program through this function (the     }
{ .double directive carries the source text), so a 1-ulp error here    }
{ miscompiles user constants.  The previous implementation multiplied  }
{ a truncated mantissa by powers of ten in double arithmetic: 1-2 ulp  }
{ off in general, and a literal with more than 19 significant digits   }
{ overflowed the Int64 mantissa and came out NEGATIVE.                 }
{                                                                      }
{ Method: Clinger fast path when both the mantissa (<= 15 digits) and  }
{ scaling power (|e| <= 22) are exactly representable, so one IEEE     }
{ multiply/divide is correctly rounded; otherwise an exact big-integer }
{ path: the significand M (up to 780 digits; 767 is the worst case     }
{ that can matter for binary64, further digits fold into a sticky      }
{ bit) is scaled by 10^E using 32-bit-limb arithmetic, and the         }
{ quotient/product bits are generated by long division with an         }
{ explicit guard bit and sticky, then rounded to nearest-even.         }
{ Handles subnormals, +-0, and overflow to +-inf exactly.              }
{ ------------------------------------------------------------------ }

const
  STD_MAXDIG = 780;    { kept significant digits; the rest is sticky }
  STD_LIMBS = 160;     { 32-bit limbs; covers 10^(324+780) < 2^3669  }

type
  TStdBig = record
    L: array[0..159] of Int64;      { STD_LIMBS entries }  { each holds 0..$FFFFFFFF }
    N: Integer;                           { limbs in use; 0 = zero  }
  end;

{ A := A * 10 + D }
procedure StdBigMul10Add(var A: TStdBig; D: Integer);
var
  I: Integer;
  Carry, T: Int64;
begin
...
function StdBigBitLen(var A: TStdBig): Integer;
var
  Top: Int64;
  Bits: Integer;
begin
  ...
  Result := (A.N - 1) * 32 + Bits;
end;

{ A := A shl K (K >= 0) }
procedure StdBigShl(var A: TStdBig; K: Integer);
var
  Words, Bits, I: Integer;
  Lo, Hi: Int64;
begin
  if (K = 0) or (A.N = 0) then
    Exit;
  Words := K div 32;
  Bits := K mod 32;
  if Bits = 0 then
  ...
  for I := A.N - 1 downto 0 do
  begin
    Hi := A.L[I] shr (32 - Bits);
    Lo := (A.L[I] shl Bits) and $FFFFFFFF;
    A.L[I + Words + 1] := A.L[I + Words + 1] or Hi;
    A.L[I + Words] := Lo;
  end;
  for I := 0 to Words - 1 do
    A.L[I] := 0;
  A.N := A.N + Words + 1;
  while (A.N > 0) and (A.L[A.N - 1] = 0) do
    A.N := A.N - 1;
end;

{ -1 / 0 / +1 for A < B / A = B / A > B }
function StdBigCmp(var A, B: TStdBig): Integer;
var
  I: Integer;
begin
  if A.N <> B.N then
...
{ A := A - B; requires A >= B }
procedure StdBigSub(var A: TStdBig; var B: TStdBig);
var
  I: Integer;
  Borrow, T: Int64;
begin
...
{ Compose the final double from a (NBits)-bit quotient Q whose last bit
  is the guard, plus a sticky flag.  E2 is the binary exponent of the
  value (leading quotient bit = 2^E2).  Handles normal, subnormal,
  overflow and the rounding carry uniformly. }
function StdCompose(Q: Int64; Sticky: Boolean; E2: Integer;
                    Neg: Boolean): Double;
var
  Mant, BitsOut: Int64;
  Guard: Int64;
begin
  Mant := Q shr 1;
  Guard := Q and 1;
  if (Guard = 1) and (Sticky or ((Mant and 1) = 1)) then
    Mant := Mant + 1;
  if E2 >= -1022 then
  begin
    if Mant = (Int64(1) shl 53) then
    begin
      Mant := Mant shr 1;
      E2 := E2 + 1;
    end;
    ...
    begin
      if Neg then
        Exit(BitsToDouble(Int64($FFF0000000000000)));
      Exit(BitsToDouble(Int64($7FF0000000000000)));
    end;
    BitsOut := (Int64(E2 + 1023) shl 52) or (Mant and $000FFFFFFFFFFFFF);
  end
  else
    { subnormal: Mant already carries the right number of bits; a
      rounding carry into bit 52 lands on the minimum normal exactly }
    BitsOut := Mant;
  if Neg then
    BitsOut := BitsOut or Int64($8000000000000000);
  Result := BitsToDouble(BitsOut);
end;
...
var
  ...
  I, C: Integer;
  Mantissa: Int64;
  Digits: array[0..779] of Integer;  { STD_MAXDIG entries }
  NSig: Integer;
  StickyTail: Boolean;
  Exp10: Integer;
  SeenDot: Boolean;
...
begin
  ...
  if (C = 110) or (C = 78) then
    Exit(BitsToDouble(Int64($7FF8000000000000)));

  { Collect significant digits.  Exp10 is maintained so that
    value = digits * 10^Exp10 with the digit string read as an
    integer; dropped digits beyond STD_MAXDIG only set StickyTail. }
  NSig := 0;
  StickyTail := False;
  Exp10 := 0;
  SeenDot := False;
  ...
  begin
    ...
    begin
      if (NSig = 0) and (C = 48) then
      begin
        { leading zero: not significant, but a fractional one scales }
        if SeenDot then
          Exp10 := Exp10 - 1;
      end
      else if NSig < STD_MAXDIG then
      begin
        Digits[NSig] := C - 48;
        NSig := NSig + 1;
        if SeenDot then
  ...
  begin
    if Neg then
      Exit(BitsToDouble(Int64(1) shl 63));
    Exit(0.0);
  end;

  { Decimal magnitude of the leading digit: 10^(TopMag-1) <= value }
  TopMag := Exp10 + NSig;
  if TopMag >= 311 then
  begin
    { >= 10^310 > DBL_MAX: overflow }
    if Neg then
      Exit(BitsToDouble(Int64($FFF0000000000000)));
    Exit(BitsToDouble(Int64($7FF0000000000000)));
  end;
  if TopMag <= -324 then
  begin
    { < 10^-324 < half the minimum subnormal: underflow }
    if Neg then
      Exit(BitsToDouble(Int64(1) shl 63));
    Exit(0.0);
  end;

  { Clinger fast path: mantissa and scale both exactly representable,
    so a single IEEE multiply or divide is correctly rounded. }
  if (not StickyTail) and (NSig <= 15) and (Exp10 >= -22) and (Exp10 <= 22) then
  begin
    Mantissa := 0;
    for K := 0 to NSig - 1 do
      Mantissa := Mantissa * 10 + Digits[K];
    Result := Mantissa * 1.0;
    if Exp10 > 0 then
  ...
  Num.N := 0;
  for K := 0 to NSig - 1 do
    StdBigMul10Add(Num, Digits[K]);

  Sticky := StickyTail;

  if Exp10 >= 0 then
  begin
    { value = Num * 10^Exp10: an exact big integer }
    for K := 1 to Exp10 do
      StdBigMul10Add(Num, 0);
    E2 := StdBigBitLen(Num) - 1;
    { generate a 54-bit quotient (53 mantissa + 1 guard) by dividing
      by 2^E2: equivalently take the top 54 bits, sticky on the rest }
    Den.N := 1;
    Den.L[0] := 1;
    StdBigShl(Den, E2);
  end
  ...
  begin
    ...
    Den.L[0] := 1;
    for K := 1 to -Exp10 do
      StdBigMul10Add(Den, 0);
    E2 := StdBigBitLen(Num) - StdBigBitLen(Den);
    { align so that Den <= Num < 2*Den, adjusting E2 }
    if E2 > 0 then
      StdBigShl(Den, E2)
    else if E2 < 0 then
      StdBigShl(Num, -E2);
    if StdBigCmp(Num, Den) < 0 then
    begin
      StdBigShl(Num, 1);
      E2 := E2 - 1;
    end;
  ...
  { number of quotient bits to generate: 53 significant + 1 guard for
    normals; fewer for subnormals (leading bit at 2^E2 = bit E2+1074
    of the subnormal field) }
  if E2 >= -1022 then
    NBits := 54
  else
    NBits := E2 + 1076;
  if NBits < 1 then
  begin
    { below half the minimum subnormal }
    if Neg then
      Exit(BitsToDouble(Int64(1) shl 63));
    Exit(0.0);
  end;

  { restoring long division: leading bit first }
  Q := 0;
  for K := 1 to NBits do
  begin
    Cmp := StdBigCmp(Num, Den);
    if Cmp >= 0 then
    begin
      Q := (Q shl 1) or 1;
      StdBigSub(Num, Den);
  ...
  Result := StdCompose(Q, Sticky, E2, Neg);
end;
