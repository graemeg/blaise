...
=== Named resource lookup over embedded files

The `{$EMBED}` / `{$EMBEDSTR}` directives (see
`docs/embedded-resources-design.adoc`) bind one file to one named constant at
compile time.  That covers the common case, but it offers no way to ask for a
resource *by name at runtime*, and no way to enumerate a set of embedded
assets.  A follow-on layer would provide that:

[source,pascal]
----
Data := FindResource('templates/report.html');
for Name in EnumResources('templates/') do
  ...
----

*Design sketch.*  This is a stdlib layer over the existing directive, not new
codegen.  A unit declares its assets with `{$EMBED}` as today, plus a registry
table mapping logical names to those constants; `FindResource` is then a lookup
over the table.  The table can be hand-written initially.  A later refinement
is to have PasBuild generate it from a `src/main/resources` tree, which would
give the Java/Maven ergonomic — `src/main/resources/report.html` addressable as
`'report.html'` with no per-file declaration — while keeping single-binary
output.

*How this differs from Java and Swift.*  Both of those resolve resources at
*runtime* through a search path (the JVM classpath; the `.app` bundle
directory), so the bytes live in a container beside the executable and can be
swapped without recompiling.  Blaise embeds at compile time instead: a missing
resource is a compile error rather than a runtime nil, and the binary stays a
single self-contained file — which matters given the freestanding-by-default
link mode.  The API surface can look similar; the semantics deliberately do
not.

*Why deferred.*  With a handful of assets, N named constants are clearer than
any lookup API, and the directive alone is sufficient.  The lookup layer only
earns its keep once a program carries many assets, or genuinely needs to
resolve a name computed at runtime.  Deferring also keeps the compiler out of
it — nothing here needs new codegen, so it can be added later without
revisiting the directive.

*Effort.*  Small-to-moderate for a hand-written registry; moderate if PasBuild
grows resource-tree generation (that also needs the resource tree folded into
the unit-cache key, in the same way `{$EMBED}` dependencies already are).
...
=== Route generic class instances through the ordinary class emitters

The three backends disagree on how a monomorphised generic class instance
reaches the metadata emitters, and the divergence has produced the same
class of bug twice.

*Current state.*  `blaise.codegen.native.arm64.pas` wraps each instance in a
synthetic `TTypeDecl` and appends it to both `FGenericDecls` (owning) and
`FClassDecls` — the list every emitter iterates — so instances flow through
the ordinary class machinery.  `blaise.codegen.native.x86_64.pas` and
`blaise.codegen.qbe.pas` instead thread the instance list as a separate
parameter and pair each class loop with a second, generic-only loop: six such
loops in the x86-64 backend and nine in the QBE backend.

*Why it matters.*  A fix applied to a class loop does not reach its generic
twin unless the author notices the pairing.  Two consecutive commits in
2026-08 fixed the identical defect — an itab omitted for a class that
*inherits* rather than declares its interface — first for plain classes, then
for generic instances, the second only after the first was reviewed.  The
arm64 backend was unaffected both times, by construction.

*What the unification costs.*  The paired loops are not duplicates.  Generic
instances must emit *weak*, *bare*, unit-independent symbols so that
duplicate monomorphisations across objects collapse at link time (see
BUG-004 in the commit history); plain classes emit strong, unit-prefixed
ones.  Today that distinction lives at the call site, which is precisely why
there are two loops.  arm64 does not remove the distinction — it relocates it
into the symbol namer, where `ClassSym` tests the type name for `<`:
...
So the refactor is not a mechanical merge.  The generic loops emit a
*sparser* set of records than the class loops — in `EmitClassSection` the
class loop emits full method metadata where the generic loop emits only a
class-name string — so unifying them means having generic instances emit what
classes emit and then re-suppressing the differences inside the naming
helpers.  That changes emitted symbols for every generic instance in the
tree.

*Assessment.*  Worth doing: it makes this bug class impossible by
construction rather than by vigilance.  Not urgent: the known instances are
fixed and covered by tests, so there is no correctness pressure.  The risk is
that a symbol-emission refactor with no intended behavioural change can pass
all five fixpoints and still ship a subtly different symbol, and the `-pre`
bootstrap binary plus every downstream clone depend on those symbols.

*Sequencing.*  Do the x86-64 backend first and alone, on its own branch — it
is the smaller half, and it establishes whether the name-sniffing approach
survives contact with a second backend before the QBE backend's 42
`ClassSymNameForDecl` sites are committed to it.

*Cheaper interim guard.*  Rather than the full refactor, a static check in
the spirit of `tools/flag-coverage` could assert that each backend's
generic-instance path emits the same symbol *kinds* as its class path.  That
is the invariant which actually broke, and it is a check rather than a
rewrite.

=== Give the AST's owned child lists an element type

Every owned child list in `uAST.pas` is an untyped `TObjectList`:
`ConstDecls`, `TypeDecls`, `ProcDecls`, `Fields`, `Methods`, `ArrayElements`
and their peers.  Each read therefore costs an unchecked downcast at the use
site — `TConstDecl(AClassDef.ConstDecls.Items[I])` — and those casts are
spread across `uAST.pas`, `uParser.pas`, `uSemantic.pas` and all three
backends.

*Why it matters.*  The cast is unchecked, so a list that is handed the wrong
node type fails at runtime rather than at compile time, and the compiler
offers no help when a walk is copied between two lists that hold different
node classes.  It also propagates: a helper extracted from such a walk must
take `TObjectList` and repeat the cast internally, because that is what the
field's declared type is — which is exactly what
`TX86_64Backend.EmitMemberConstBlobs` had to do when the class and record
member-const walks were unified (BUG-20260922-record-const-array-unresolved-symbol).

*What it would take.*  The project already mandates `Generics.Collections`
for new collection use, so the destination type exists.  The obstacle is
ownership: `TObjectList.Create(True)` frees its items, and the AST's
destructors rely on that for teardown.  `TList<T>` has no ownership
semantics, so a direct substitution would require an explicit free loop in
every AST destructor — replacing one implicit contract with a hand-written
one.  The clean form is an owning generic list (`TObjectList<T>`, or an
`OwnsObjects` flag on a generic list type) added to
`generics.collections.pas` first; the AST change is mechanical after that.

*Risk.*  `uAST.pas` is compiled by every stage of the self-hosting chain, so
introducing a new generic instantiation there is the shape most likely to
trip the teach-then-use rule and produce a `BOOTSTRAP_BROKEN` in CI.  The
generic list type must land, and be in a published release binary, before
the AST starts using it.

*Assessment.*  Worth doing for type safety and to shrink the cast surface,
but it is a two-step change gated on a release boundary, and it has no
correctness pressure behind it — the existing casts are all correct today.
