...
The header is `BLAISE-IFACE <version>`, where the version field is the
constant `IFACE_VERSION` in `uUnitInterfaceIO.pas`.

That constant is the authority and is bumped whenever a payload layout
grows, so this document deliberately does not restate its current value —
read it from `uUnitInterfaceIO.pas`, where the declaration carries the
per-version changelog. (The first bump, `1` → `2`, added `CompilerId` and
`SourceModTime` to the `META` block; see <<versioning>> for the rule.)
...
* `method-decl-list` (from `EncodeMethodDecl`) lacks the resolved
  ...
  *and* by generic-routine templates (`GENROUT`), which is why it must
  still carry the declaration-level directives (`IsVirtual`,
  `IsOverride`, `IsOverload`, `IsStatic`): a `static` method on a
  generic reaches an importing unit only through this path.
...
. **Content drift.** Even with a matching version, a `.bif` may be
  ...
  ** **Source available on path** → recompute the FNV-1a hash and, if it
     matches `SourceHash`, accept; otherwise recompile from source. The
     hash covers the source bytes *and* the content of every file the
     source pulls in with `{$EMBED}` / `{$EMBEDSTR}`
     (`SourceHashWithEmbeds`), so editing an embedded asset invalidates
     the entry exactly as editing the source does. Discovery lexes the
     source, so it is given the same `-d`/`--define` set the compile uses
     — otherwise a define-gated embed resolves to the other branch and
     silently escapes the key.
