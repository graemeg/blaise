...
NOTE: Done, then superseded. FreeBSD was originally pinned to Strategy B by a
`TargetIsFreestanding(target)` predicate, so a `freebsd-x86_64` target linked
static `ET_EXEC` / freestanding `_start` / no `PT_INTERP` / no libc regardless
of `--static`. That predicate has since been deleted: FreeBSD now follows the
same auto link policy as Linux (freestanding by default, dynamic + libc when
the program binds an external C library, forced either way by `--static` /
`--dynamic`) — see the "Link modes" section of
`docs/toolchain-independence.adoc`. The per-target `TPlatformLayout` is
injected by the compiler as a
