...
* GNU `make` — only for building the vendored QBE backend. The compiler builds
  and links its RTL from source, so plain compiler use needs no `make` at all.
...
Bootstrapping is a single step — the compiler compiles the RTL from source on
demand, so there is no separate runtime build to do first:
...
To bootstrap via the opt-in QBE backend instead, build the vendored QBE once
...
$RELEASE \
  ...
  --unit-path compiler/src/main/pascal \
  --unit-path stdlib/src/main/pascal \
  --backend qbe --output compiler/target/blaise
----
...
[NOTE]
====
`--emit-ir` / `--emit-asm` are *debugging aids*, not build steps: they print
the generated IR/assembly and produce no binary.  Always build with
`--output`, which lets the compiler own the whole pipeline.

In particular, do **not** hand-link IR with `qbe` + `gcc`.  The compiler
compiles the RTL from source on demand into a target-keyed object cache beside
its own binary, so a manual link is both unnecessary and easy to get wrong (it
was the cause of https://github.com/graemeg/blaise/issues/180[issue #180]).

What the compiler *does* need is the RTL source to be reachable — via
`--rtl-src DIR`, `$BLAISE_RTL_SRC`, or a `blaise.cfg` beside the binary (the
release tarballs ship one, so an extracted release works with zero flags).
====
